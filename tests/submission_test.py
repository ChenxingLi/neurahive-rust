...
class SubmissionTest(TestFramework):
    ...
    def submission_data(self, size, submission_index, rand_data=True):
        ...
        assert_equal(client.nrhv_get_file_info(data_root)["finalized"], False)
        ...
        assert_equal(
            base64.b64decode(
                client.nrhv_download_segment(data_root, 0, 1).encode("utf-8")
            ),
            first_entry,
        )
        ...
        for i in range(0, self.num_nodes):
            ...
            assert_equal(
                base64.b64decode(
                    self.nodes[i]
                    .nrhv_download_segment(data_root, 0, 1)
                    .encode("utf-8")
                ),
                first_entry,
            )
