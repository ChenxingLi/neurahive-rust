...
class CrashTest(TestFramework):
    ...
    def run_test(self):
        ...
        wait_until(lambda: self.nodes[0].nrhv_get_file_info(data_root) is not None)
        ...
        for i in range(self.num_nodes):
            ...
            wait_until(
                lambda: self.nodes[i].nrhv_get_file_info(data_root) is not None
            )
            wait_until(
                lambda: self.nodes[i].nrhv_get_file_info(data_root)["finalized"]
            )
        ...
        segment = submit_data(self.nodes[0], chunk_data)
        wait_until(lambda: self.nodes[0].nrhv_get_file_info(data_root)["finalized"])

        self.start_storage_node(1)
        self.nodes[1].wait_for_rpc_connection()
        self.nodes[1].admin_start_sync_file(1)
        wait_until(lambda: self.nodes[1].nrhv_get_file_info(data_root) is not None)
        wait_until(lambda: self.nodes[1].nrhv_get_file_info(data_root)["finalized"])
        ...
        wait_until(lambda: self.nodes[0].nrhv_get_file_info(data_root1) is not None)
        ...
        wait_until(lambda: self.nodes[0].nrhv_get_file_info(data_root1)["finalized"])
        ...
        for i in range(1, self.num_nodes):
            ...
            wait_until(
                lambda: self.nodes[i].nrhv_get_file_info(data_root1) is not None
            )
            wait_until(
                lambda: self.nodes[i].nrhv_get_file_info(data_root1)["finalized"]
            )
