...
class RpcTest(TestFramework):
    ...
    def run_test(self):
        ...
        wait_until(lambda: client1.nrhv_get_file_info(data_root) is not None)
        assert_equal(client1.nrhv_get_file_info(data_root)["finalized"], False)

        wait_until(lambda: client2.nrhv_get_file_info(data_root) is not None)
        assert_equal(client2.nrhv_get_file_info(data_root)["finalized"], False)

        segment = submit_data(client1, chunk_data)
        self.log.info("segment: %s", segment)

        wait_until(lambda: client1.nrhv_get_file_info(data_root)["finalized"])
        assert_equal(
            client1.nrhv_download_segment(data_root, 0, 1), segment[0]["data"]
        )

        client2.admin_start_sync_file(0)
        wait_until(lambda: client2.sycn_status_is_completed_or_unknown(0))

        wait_until(lambda: client2.nrhv_get_file_info(data_root)["finalized"])
        assert_equal(
            client2.nrhv_download_segment(data_root, 0, 1), segment[0]["data"]
        )

        self.__test_upload_file_with_cli(client1)

        client2.shutdown()
        wait_until(lambda: client1.nrhv_get_status() == 0)
    ...
    def __test_upload_file_with_cli(self, client1):
        ...
        for size in [2, 1023, 2051]:
            ...
            wait_until(lambda: client1.nrhv_get_file_info(root) is not None)
            wait_until(lambda: client1.nrhv_get_file_info(root)["finalized"])
            ...
            for i in range(1, self.num_nodes):
                ...
                wait_until(lambda: self.nodes[i].nrhv_get_file_info(root) is not None)
                wait_until(
                    lambda: self.nodes[i].nrhv_get_file_info(root)["finalized"]
                )

                assert_equal(
                    client1.nrhv_download_segment(root, 0, 2),
                    self.nodes[i].nrhv_download_segment(root, 0, 2),
                )
