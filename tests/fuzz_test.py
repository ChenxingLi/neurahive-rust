    ...
    def run_test(self):
        ...
        def run_chunk_size(sizes, nodes, contract, log):
            ...
            for i, v in enumerate(sizes):
                ...
                wait_until(
                    lambda: nodes[idx].nrhv_get_file_info(data_root) is not None
                )
                segment = submit_data(nodes[idx], chunk_data)
                wait_until(
                    lambda: nodes[idx].nrhv_get_file_info(data_root)["finalized"]
                )
            ...
            for data_root in data_roots:
                for idx in range(len(nodes)):
                    ...
                    def wait_finalized():    
                        ret = nodes[idx].nrhv_get_file_info(data_root)
                        if ret["finalized"]:
                            return True
                        else:
