...
class RandomTest(TestFramework):
    def setup_params(self):
        ...
        for i in range(self.num_nodes):
            self.nrhv_node_configs[i] = {"find_peer_timeout_secs": 1, "confirmation_block_count": 1}
    ...
    def run_test(self):
        ...
        for i in range(tx_count):
            ...
            wait_until(lambda: client.nrhv_get_file_info(data_root) is not None, timeout=120)
            if not no_data:
                submit_data(client, chunk_data)
                wait_until(lambda: client.nrhv_get_file_info(data_root)["finalized"])
                # Wait until the tx is sent out.
                for node_index in range(len(self.nodes)):
                    if node_index != chosen_node:
                        self.log.debug(f"check {node_index}")
                        wait_until(lambda: self.nodes[node_index].nrhv_get_file_info(data_root) is not None, timeout=120)
                        wait_until(lambda: self.nodes[node_index].nrhv_get_file_info(data_root)["finalized"], timeout=300)
        ...
        for i in range(tx_count):
            for node in self.nodes:
                status = node.nrhv_get_file_info_by_tx_seq(i)
        ...
        for data_root, no_data in file_list:
            if not no_data:
                for node in self.nodes:
                    ...
                    wait_until(lambda: node.nrhv_get_file_info(data_root)["finalized"], timeout=300)
