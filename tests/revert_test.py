...
class RevertTest(TestFramework):
    ...
    def run_test(self):
        ...
        assert client.nrhv_get_file_info(data_root) is None
        # Generate blocks for confirmation
        blockchain_client1.generate_empty_blocks(12)
        wait_until(lambda: client.nrhv_get_file_info(data_root) is not None)
        ...
        wait_until(lambda: client.nrhv_get_file_info(data_root) is None)
        wait_until(lambda: client.nrhv_get_file_info(data_root1) is not None)
        wait_until(lambda: not client.nrhv_get_file_info(data_root1)["finalized"])

        segment = submit_data(client, chunk_data)
        self.log.info("segment: %s", segment)
        wait_until(lambda: client.nrhv_get_file_info(data_root1)["finalized"])
