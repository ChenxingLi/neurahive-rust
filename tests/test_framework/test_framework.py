...
from test_framework.nrhv_node import NrhvNode
...
class TestFramework:
    ...
    def __setup_nrhv_node(self):
        for i in range(self.num_nodes):
            if i in self.nrhv_node_configs:
                updated_config = self.nrhv_node_configs[i]
            else:
                updated_config = {}

            assert os.path.exists(self.nrhv_binary), (
                "%s should be exist" % self.nrhv_binary
            )
            node = NrhvNode(
                i,
                self.root_dir,
    ...
    def main(self):
        ...
        if self.options.tmpdir:
            ...
            self.options.tmpdir = os.getenv(
                "NRHV_TESTS_LOG_DIR", default=tempfile.mkdtemp(prefix="nrhv_test_")
            )
        ...
        self.nrhv_binary = self.options.neurahive
