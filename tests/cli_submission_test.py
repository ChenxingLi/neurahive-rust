...
class CliSubmissionTest(TestFramework):
    ...
    def __test_upload_file_with_cli(self, size, submission_index, rand_data=True):
        ...
        wait_until(lambda: client.nrhv_get_file_info(root) is not None)
        wait_until(lambda: client.nrhv_get_file_info(root)["finalized"])
        ...
        if num_of_entris > 1:
            ...
            assert_equal(
                client.nrhv_download_segment(root, start_idx, end_idx),
                base64.b64encode(
        ...
        for i in range(0, self.num_nodes):
            ...
            wait_until(lambda: self.nodes[i].nrhv_get_file_info(root)["finalized"])
            ...
            assert_equal(
                client.nrhv_download_segment(root, 0, 1),
                self.nodes[i].nrhv_download_segment(root, 0, 1),
            )
