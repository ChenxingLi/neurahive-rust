...
class SubmissionTest(TestFramework):
    ...
    def run_test(self):
        ...
        for i, v in enumerate(data_size):
            ...
            for tx_offset in range(same_root_tx_count + 1):
                ...
                status = self.nodes[0].nrhv_get_file_info_by_tx_seq(tx_seq)
    ...
    def submit_tx_for_data(self, chunk_data, tx_seq, data_finalized=False, node_idx=0):
        ...
        wait_until(lambda: client.nrhv_get_file_info_by_tx_seq(tx_seq) is not None)
        assert_equal(client.nrhv_get_file_info_by_tx_seq(tx_seq)["finalized"], data_finalized)
