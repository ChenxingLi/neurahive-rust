...
extern crate miner as nrhv_miner;
...
use nrhv::RpcServer as NrhvRpcServer;
use nrhv_miner::MinerMessage;
use std::error::Error;
use std::sync::Arc;
...
pub async fn run_server(ctx: Context) -> Result<HttpServerHandle, Box<dyn Error>> {
    ...
    let mut nrhv = (nrhv::RpcServerImpl { ctx: ctx.clone() }).into_rpc();
