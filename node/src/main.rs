...
use crate::config::NrhvConfig;
use client::{Client, ClientBuilder, RuntimeContext};
use std::error::Error;

async fn start_node(context: RuntimeContext, config: NrhvConfig) -> Result<Client, String> {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let config = NrhvConfig::parse(&matches)?;
    log::configure(
    ...
    executor.clone().spawn(
        async move {
            ...
            if let Err(e) = start_node(context.clone(), config).await {
                error!(reason = %e, "Failed to start nrhv node");
