...
use crate::{try_option, NrhvKeyValueDB};
use anyhow::{anyhow, bail, Result};
use append_merkle::{MerkleTreeInitialData, MerkleTreeRead};
use itertools::Itertools;
use nrhv_spec::{BYTES_PER_SECTOR, SEALS_PER_LOAD, SECTORS_PER_LOAD, SECTORS_PER_SEAL};
...
impl FlowStore {
    pub fn new(db: Arc<dyn NrhvKeyValueDB>, config: FlowConfig) -> Self {
        Self {
...
pub struct FlowDBStore {
    kvdb: Arc<dyn NrhvKeyValueDB>,
}

impl FlowDBStore {
    pub fn new(kvdb: Arc<dyn NrhvKeyValueDB>) -> Self {
        Self { kvdb }
    }
